...
import fastmath
from fastmath import nested_map
from fastmath import numpy as jnp
...
class Layer:
  ...
  def _do_custom_gradients(self, x):
    ...
    do_forward = fastmath.custom_vjp(
        _f, _f_fwd, _f_bwd, nondiff_argnums=(0, 1))
...
def _short_traceback(skip=3):
  ...
  lines = traceback.format_exc(chain=False).splitlines()[
      skip:]  # pylint: disable=unexpected-keyword-arg
  for l in lines:
